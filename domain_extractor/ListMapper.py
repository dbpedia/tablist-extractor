...
import sys
import time
from domain_extractor.mapping_rules import *

class ListMapper:

    def __init__(self, resDict, language, resource, res_class, graph, utils):
        ...
        self.db_res = rdflib.URIRef(self.dbr + self.resource)

        self.mapped_domains = []  # used to prevent duplicate mappings
    ...
    def select_mapping(self):
        ...
        if self.mappers:
            for mapper in self.mappers:
                ...
                domain_keys = []
                try:
                    if self.language in self.dictionary[mapper]["list_headers"]:
                    ...
                    try:
                        if self.language in eval(mapper):
                            domain_keys = eval(mapper)[self.language]  # e.g. ['bibliography', 'works', ..]
                ...
                for res_key in list(self.resDict.keys()):  # iterate on resource dictionary keys
                    mapped = False
                        ...
                        if not mapped and re.search(dk, res_key, re.IGNORECASE):
                            try:
                                if is_custom_map_fn == False:
                                    ...
                                    res_elems += eval(mapperfn)  # calls the proper mapping for that domain and counts extracted elements
                                    mapped = True  # prevents the same section to be mapped again
                                else:
                                    mapper_elems = self.map_user_defined_mappings(mapper, self.resDict[res_key], res_key, self.db_res, self.language, self.graph, 0)
        ...
        * Finally, it applies related mapping functions for the list elements contained in that section.
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_user_defined_mappings(mapper_fn_name, elem, sect_name, res, lang, g, elems)   # handle recursive lists

            else:
                #elem = elem.decode('utf-8')  # apply utf-8 encoding
                years = []
                ...
                if ontology_class == None:   # No possible mapping found; try default mapping
                    try:
                        if "default" not in list(mapper_settings["ontology"][lang].keys()):
                ...
                try:
                    if res_name == None and 1 in extractor_choices: # italics mapper was chosen
                        ...
                        if res_name:
                            ...
                            res_name = urllib.parse.quote(res_name)  # quoting res_name in proper format
                            uri = self.dbr + res_name
                    ...
                    if res_name == None and 2 in extractor_choices: # reference mapper was chosen
                        ...
                        if res_name:  # current element contains a reference
                            ...
                            if uri:
                                ...
                                uri_name = urllib.parse.quote(uri_name)  # quoting res_name in proper format
                                uri = self.dbr + uri_name
                    ...
                    if uri and uri != "":
                        ...
                        elems += 1
                        if isbn:
                            g.add((rdflib.URIRef(uri), self.dbo.isbn, rdflib.Literal(isbn, datatype=rdflib.XSD.string)))
                        if work:
                            g.add((rdflib.URIRef(uri), self.dbo.notableWork, rdflib.Literal(work, datatype=rdflib.XSD.string)))
                        if years:
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                ...
                if res_name:
                    elem = elem.replace(res_name, "")  # delete resource name found from element for further mapping
                    res_name = res_name.replace(' ', '_')
                    res_name = urllib.parse.quote(res_name)  ###
                    uri = self.dbr + res_name
                    ...
                    if ref:  # current element contains a reference
                        ...
                        if uri:
                            ...
                            uri_name = urllib.parse.quote(uri_name)  
                            uri = self.dbr + uri_name
                    
                    else:  # no reference found, try general mapping (less accurate)
                        uri_name = self.general_mapper(elem)
                ...
                if uri and uri != "":
                    ...
                    elems += 1
                    if year:
                        self.add_years_to_graph(g, uri, year, {'activeYear':'releaseYear'})

        return elems


    def map_concert_tours(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_concert_tours(elem, sect_name, res, lang, g, elems)   # handle recursive lists
                ...
                if uri and uri != "":
                    ...
                    elems += 1
                    if year:
                        self.add_years_to_graph(g, uri, year)

        return elems


    def map_alumni(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_alumni(elem, sect_name, res, lang, g, elems)   # handle recursive lists
    ...
    def map_programs_offered(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_programs_offered(elem, sect_name, res, lang, g, elems)   # handle recursive lists
                ...
                # add successfuly extracted triples into the graph
                if uri and uri != "":
                    g.add((rdflib.URIRef(uri), self.dbo.academicDiscipline, res))
                    elems+=1
                
        return elems


    def map_honors(self, elem_list, sect_name, res, lang, g, elems):
        ...
        award_status = self.award_status_mapper(sect_name, lang)  # if award status is found in the section name.
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_honors(elem, sect_name, res, lang, g, elems)
                ...
                if uri and uri != "":
                    ...
                    if year:
                        self.add_years_to_graph(g, uri, year)
                    if for_entity:
                        g.add((rdflib.URIRef(uri), self.dbo.AwardedFor, self.dbr + rdflib.URIRef(for_entity)))
                    if from_entity:
                        g.add((self.dbo + rdflib.URIRef(award_status), self.dbo.AwardedBy, self.dbr + rdflib.URIRef(from_entity)))
    ...
    def map_staff(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_staff(elem, sect_name, res, lang, g, elems)   # handle recursive lists
                ...
                if uri and uri != "":
                    if len(list(g.triples((rdflib.URIRef(uri), self.dbo.alumni, res)))) == 0 and \
                        ...
                        g.add((rdflib.URIRef(uri), self.dbo.staff, res))
        
        return elems


    def map_other_person_details(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_other_person_details(elem, sect_name, res, lang, g, elems)   # handle recursive lists
    ...
    def map_career(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_career(elem, sect_name, res, lang, g, elems)
                ...
                uri_name = self.quote_mapper(elem)
                if uri_name == None or uri_name == res: uri_name = self.general_mapper(elem)
                if (uri_name and uri_name != "" and uri_name != res):
                    uri_name = uri_name.replace(' ', '_')
                    uri_name = urllib.parse.quote(uri_name)  ###
                    uri = self.dbr + uri_name
    ...
    def map_filmography(self, elem_list, sect_name, res, lang, g, elems):
            ...
            if type(elem) == list:  # for nested lists (recursively call this function)
                ...
                if uri and uri != "":
                    ...
                    if year:
                        self.add_years_to_graph(g, uri, year, {'activeYear':'releaseYear'})
                    if film_particip:
                        g.add((rdflib.URIRef(uri), self.dbo + rdflib.URIRef(film_particip), res))
                    elems += 1
        
        return elems


    def map_bibliography(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_bibliography(elem, sect_name, res, lang, g, elems)
                ...
                if uri and uri != "":
                    ...
                    if year:
                        self.add_years_to_graph(g, uri, year)
                    if lit_genre:
                        g.add((rdflib.URIRef(uri), self.dbo.literaryGenre, self.dbo + rdflib.URIRef(lit_genre)))
        
        return elems


    def map_band_members(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_members(elem, sect_name, res, lang, g, elems)
    ...
    def map_contributors(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_contributors(elem, sect_name, res, lang, g, elems)
                ...
                for t in list(CONTRIBUTION_TYPE[lang].keys()):
                ...
                if uri and uri != "":
                    if contrib_type:
                        ...
                        g.add((rdflib.URIRef(uri), self.dbo.ContributedTo, res))
                    if year:
                        self.add_years_to_graph(g, uri, year)
    ...
    def map_other_literature_details(self, elem_list, sect_name, res, lang, g, elems):
        ...
        for elem in elem_list:
            if type(elem) == list:  # for nested lists (recursively call this function)
                elems += 1
                self.map_other_literature_details(elem, sect_name, res, lang, g, elems)

            else:
                detail_type = None
                # if subsection exists, select corresponding ontology class from OTHER_LITERATURE_DETAILS
                for t in list(OTHER_LITERATURE_DETAILS[lang].keys()):
                ...
                if uri and uri != "":
                    if detail_type:
                        ...
                        g.add((rdflib.URIRef(uri), self.dbo.WrittenWork, res))
                    if year:
                        self.add_years_to_graph(g, uri, year)
    ...
    def add_years_to_graph(self, g, uri, year, year_ontology = {}):
        ...
        * Empty dict indicates default values should be taken from y_ontology.
        ...
        for ontology in list(year_ontology.keys()):
            ...
            for y in year:
                if type(y) != list:  # Single date (not a time-period)
                    ...
                    if "^" in y[0]:   # '^' is a seperator used for month and year
                        d = y[0].replace("^","-")
                        g.add((rdflib.URIRef(uri), self.dbo[y_ontology['activeYearsStartDate']], rdflib.Literal(d, datatype=rdflib.XSD.gYearMonth)))
                    else:    
                        g.add((rdflib.URIRef(uri), self.dbo[y_ontology['activeYearsStartDate']], rdflib.Literal(y[0], datatype=rdflib.XSD.gYear)))

                    if "^" in y[1]:
                        d = y[1].replace("^","-")
                        g.add((rdflib.URIRef(uri), self.dbo[y_ontology['activeYearsEndDate']], rdflib.Literal(d, datatype=rdflib.XSD.gYearMonth)))

                    else:    
                        g.add((rdflib.URIRef(uri), self.dbo[y_ontology['activeYearsEndDate']], rdflib.Literal(y[1], datatype=rdflib.XSD.gYear)))
    ...
    def alumni_profession_mapper(self, list_elem):
    ...
    def isbn_mapper(self, list_elem):
    ...
    def year_mapper(self, list_elem):
    ...
    def month_year_mapper(self, list_elem):
    ...
    def litgenre_mapper(self, sect_name, lang):
        ...
        for bg in list(b_genres.keys()):  # iterate on literary genres provided for the given language
            if re.search(bg, sect_name, re.IGNORECASE):  # try to match section name with a genre
                for other_bg in list(b_genres.keys()):
    ...
    def filmpart_mapper(self, sect_name, lang):
    ...
    def filmtype_mapper(self, sect_name, lang):
        ...
        for ft in list(f_types.keys()):
            if re.search(ft, sect_name, re.IGNORECASE):
                filmtype = f_types[ft]
        
        return filmtype

    def award_status_mapper(self, sect_name, lang):
        ...
        for st in list(s_types.keys()):
            if re.search(st, sect_name, re.IGNORECASE):
                status = s_types[st]
        
        return status

    def sentence_splitter(self, elem,word,lang):
    ...
    def bracket_feature_mapper(self, elem):
        ...
        '''
        entity = None
        val = re.search(r'\(.*\)', elem, re.IGNORECASE)
        if val:
            val = val.group()
    ...
    def wikidataAPI_call(self, res, lang):
        ...
        except urllib.error.URLError:  # sometimes the host can refuse too many connections and returns a socket error
            time.sleep(5)  #wait 5 seconds and then retry
            print("retrying Wikidata API call...")
            self.wikidataAPI_call(res, lang)
    ...
    def find_DBpedia_uri(self, wk_uri, lang):
        ...
        try:
            json = self.utils.sparql_query(query, lang)
        
        except IOError:
            time.sleep(5)
            print("retrying DBpedia API call...")
            self.find_DBpedia_uri(wk_uri, lang)
    ...
    def list_elem_clean(self, list_elem):
    ...
    def italic_mapper(self, list_elem):
        ...
        if match_italic:
            ...
            match_italic = self.list_elem_clean(match_italic)
        
        return match_italic


    def reference_mapper(self, list_elem):
        ...
        if match_ref:
            ...
            if match_num:  # date references must be ignored for this mapping
                ...
                match_ref = self.reference_mapper(new_ref)  #call again reference_mapper passing the new reference
        
        return match_ref


    def general_mapper(self, list_elem):
        ...
        list_elem = self.list_elem_clean(list_elem)
        ...
        if match_str != None:
            ...
            match_str = self.list_elem_clean(match_str)
