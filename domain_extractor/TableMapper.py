...
class TableMapper:
    ...
    def __init__(self, language, graph, domain, resource, table_data, utils, mappers, index, table_section=None):
        """
        ...
        self.mapperTools = self.utils.mapper
    ...
    def map(self):
        ...
        for mapper in self.mappers:
            if mapper not in explored_mappers and mapper in self.dictionary and self.is_section_in_mapper(self.table_section, mapper):
                # link with between resource section property and each section's row
                section_property = self.search_on_dictionary(self.table_section, mapper, research_type='section')
                ...
                if section_property:
                    ...
                    for row in self.table_data:
                        ...
                        for cell in row:
                            # read value
                            value = self.mapperTools.extract_value_from_cell(row[cell])
                            # character "-" means that cell is empty
                            if value != '-':
                                # get property of table's header
                                dictionary_property = self.search_on_dictionary(cell, mapper, research_type='cell')
    ...
    def search_on_dictionary(self, key, mapper, research_type):
        """
        ...
        if research_type == "section":
            value = self.dictionary[mapper]['ontology'][self.language][key]
            ...
            if strict_rule in self.dictionary[mapper]['ontology'][self.language]:
                value = self.dictionary[mapper]['ontology'][self.language][key]
                message = "Header: " + key + " mapped with: " + str(value)
                key_found = strict_rule
            # less strict rule means a research of only 'key' in dictionary
            elif key in self.dictionary[mapper]['ontology'][self.language]:
                value = self.dictionary[mapper]['ontology'][self.language][key]
                message = "Header: " + key + " mapped with: " + str(value)
                key_found = key
            else:
    ...
    def is_section_in_mapper(self, table_section, mapper):
        if table_section in self.dictionary[mapper]['table_sections'][self.language]:
            return True
        else:
            return False
    ...
    def check_value_type(self, value):
        ...
        result = value
        if self.mapperTools.is_float(result):
            data_type = rdflib.namespace.XSD.float
        elif self.mapperTools.is_int(result):
    ...
    def check_if_is_resource(self, resource):
        ...
        resource_to_search = self.mapperTools.adjust_resource(resource)
